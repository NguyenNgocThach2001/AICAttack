...
if __name__ == '__main__':
    ...
    num_input = DEFAULT_SAMPLE_COUNT
    attens = []
    random.seed(42)
    ids = random.sample(range(DEFAULT_SAMPLE_RANGE), num_input)
    for i in ids:
        attens.append((i, get_alphas(i,dataset='flicker8k')))
        print(f'No.{i}')
    # Open the file in write mode
    with open(f"attens_flicker8k_{num_input}samples_{DEFAULT_ALPHA_SIZE}.json", "wb") as file:
